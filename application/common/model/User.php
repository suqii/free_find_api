...
class User extends Model
{
    ...
    public function sendCode()
    {
        ...
        if (!config('api.aliSMS.isopen')) {
            ...
            TApiException('验证码：'.$code, false, 200);
        }
        // 发送验证码
        ...
        if ($res['Code']=='isv.MOBILE_NUMBER_ILLEGAL') {
            TApiException('无效号码', 30002);
        }
        // 触发日限制
        if ($res['Code']=='isv.DAY_LIMIT_CONTROL') {
            TApiException('今日你已经发送超过限制，改日再来', 30003);
        }
        // 发送失败
    ...
    public function userCreate()
    {
        ...
        if (!$user) {
            ...
            'username'=>'测试用户名',
            'phone'=>'18882076569',
            'password'=>'18882076569',
            'userpic'=>'https://img1.baidu.com/it/u=2441904516,161462902&fm=26&fmt=auto&gp=0.jpg',
            ]);
        }
        return $user;
    }
    ...
    public function getNewUser()
    {
        return $this->alias('u')
            ...
            ->order('u.id desc')
            ->field('u.username,u.create_time as createAt,i.sex as status')
            ->limit(10)
            ->select();
    }
    // 获取近七天新增数据
